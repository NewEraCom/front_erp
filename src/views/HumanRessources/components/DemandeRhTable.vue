...
const actionsConfig = [
  ...
  {
    icon: 'ti ti-trash-filled',
    type: 'delete',
    class: 'btn btn-danger btn-sm',
    onClick: (item: any) => deleteItem(item)
  },
  {
    icon: 'ti ti-check',
    class: 'btn btn-success btn-sm',
    onClick: (item: any) => ReceivedItem(item)
  },
  { icon: 'ti ti-plus', class: 'btn btn-info btn-sm', onClick: (item: any) => Addattachement(item) }
]
...
const Addattachement = (item: any) => {
  console.log('Received item', item)
  rhStore.setItemId(item.id)
  $('#attachement-modal').modal('show')
}
...
const filter = () => {
  filteredData.value = props.demandes.filter((item: any) => {
    const combinedFields =
      `${item.employe.first_name} ${item.employe.last_name} ${item.post_name} ${item.diploma} ${item.experience}`.toLowerCase()

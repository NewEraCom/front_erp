...
const headers = [
    { text: 'Raison sociale', value: 'raison_social', type: 'client' },
    { text: 'Ville', value: 'ville', type: 'text' },
    { text: 'Delai de paiement', value: 'd_paiment', type: 'days' },
    { text: 'Numéro de téléphone', value: 'phone', type: 'phone' },
...
const filter = () => {
    filteredData.value = props.clients.filter((item: any) => {
        const combinedFields = `${item.raison_social} ${item.ville} ${item.num_rc} ${item.phone} ${item.email}`.toLowerCase();

...
const submit = async () => {
  await financeService
    ...
    .then(() => {
      $('#addeffet').modal('hide')
      toast.success('Effet ajouté avec succès')
    })
    .catch((error) => {
      console.log(error)
      toast.error(error)
...
<template>
  <Modal id="addeffet" title="Nouveau Effet" size="modal-lg">
    <form @submit.prevent="submit">
      <div class="modal-body">
        <div class="row">
          <div class="col-6 mb-3">
            <label for="numero" class="mb-2"
              >Entrez le numero d' Effet<span class="text-danger">*</span></label
            >
            <input
              type="text"
              class="form-control"
              v-model="cheque.numero"
              placeholder="Entrez le numero d'Effet"
              required
            />
          </div>
          <div class="col-6 mb-3">
            ...
            <select class="form-select" v-model="cheque.carnet_id" required>
              <option
                v-if="carnets !== null"
                v-for="carnet in carnets.filter((carnet) => carnet.type === 'effet')"
                :key="carnet.id"
                :value="carnet.id"
              >
                {{ carnet.numero }}

      ...
      <tbody v-if="paginatedItems.length > 0">
        <tr v-for="item in paginatedItems" :key="item.id">
          <template v-for="(header, index) in headers" :key="header.value">
            <td
              v-if="header.isComplex && header.type === 'employee'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
            ...
            <td
              v-if="header.isComplex && header.type === 'fullname'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
            ...
            <td
              v-if="header.isComplex && header.type === 'workers'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
            ...
            <td
              v-if="header.isComplex && header.type === 'project'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
            ...
            <td v-if="header.isComplex && header.type === 'preProjects'">
              ...
              <small class="fw-bold text-muted">
                ...
                <i
                  v-if="item.type_project != 'simple' && item.lots.length == 0"
                  class="ti ti-alert-circle-filled text-danger"
                />
              </small>
            </td>
            <td
              v-if="header.isComplex && header.type === 'preproject'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
              <!-- <h6 class="mb-1 fw-bold">{{ item.name }}</h6> -->
            ...
            <td
              v-if="header.isComplex && header.type === 'leave'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
              <router-link :to="{ name: 'ProfileEmployee', params: { id: item.employe.id } }">
                <h6 class="mb-1 fw-bold text-primary">
                  {{ item.employe.first_name + ' ' + item.employe.last_name }}
                </h6>
              </router-link>
              <small class="fw-bold text-muted">Matricule : {{ item.employe.matricule }}</small>
            </td>
            <td
              v-if="header.isComplex && header.type === 'SousTraitant'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
              <router-link :to="{ name: 'ProfileSoustraitant', params: { id: item.id } }">
                <h6 class="mb-1 fw-bold text-primary">{{ item.raison_social }}</h6>
              </router-link>
            </td>
            <td
              v-if="header.isComplex && header.type === 'recrute'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
            ...
            <td v-if="!header.isComplex" :class="index == 0 ? 'text-start' : 'text-center'">
              ...
              <small v-else>
                ...
                <span v-if="header.type === 'achat'"> N° {{ item.achat.n_order }} </span>
                <span v-if="header.type === 'bdc'"> N° {{ item.bon_commande.num }} </span>
                <span v-if="header.type === 'phone'">
                  {{ formater.phoneNumber(item[header.value]) }}
                </span>
                <span v-if="header.type === 'facture'">
                  <small v-if="item[header.value] == '-' || item[header.value] != null"
                    >Aucun facture
                  </small>
                ...
                <span v-if="header.type === 'attachement'">
                  <small v-if="item[header.value] === 'NULL'">Aucun Attachement</small>
                  <button v-else class="btn btn-label-primary btn-sm">
                    <i class="ti ti-download me-2"></i
                    ><a
                      :href="`${uploadsUrl}storage/` + item[header.value]"
                      >Télécharger l'attachement</a
                    >
                  </button>
                </span>
                ...
                <span v-if="header.type === 'caisse'">
                  ...
                  <span v-else>
                    <h6 class="mb-1 fw-bold">
                      {{ item.recepteur.first_name + ' ' + item.recepteur.last_name }}
                    </h6>
                ...
                <span v-if="header.type == 'client'">
                  <router-link to="/">
                    <h6 class="mb-1 fw-bold text-primary">
                      {{ formater.limitText(item.raison_social, 30) }}
                    </h6>
                  </router-link>
            ...
            <td
              v-if="header.isComplex && header.type === 'tier'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
              <!-- <h6 class="mb-1 fw-bold">{{ item.name }}</h6> -->
              <small class="fw-bold text-muted">Code : {{ item.tier.raison_social }}</small>
            </td>
            <td
              v-if="header.isComplex && header.type === 'carnet'"
              :class="index == 0 ? 'text-start' : 'text-center'"
            >
          ...
          <td v-if="buttonType == 'simple'" class="text-center">
            ...
            >
              <i
                v-if="action.type == 'potential'"
                :class="item.potentiel === '1' ? 'ti ti-bookmark-filled' : 'ti ti-bookmark'"
              ></i>

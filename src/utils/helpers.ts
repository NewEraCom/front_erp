...
function calculateDifference(item: any, workingHours: number) {
  ...
  const totalShiftDuration =
    (endHour - startHour) * 60 * 60 * 1000 + (endMinute - startMinute) * 60 * 1000;
  const totalBreakDuration =
    (breakEndHour - breakStartHour) * 60 * 60 * 1000 +
    (breakEndMinute - breakStartMinute) * 60 * 1000;
...
// const complexTypeToString = (item, type) => {
//   switch (type) {
//     case 'employee':
...
//     case 'preproject':
//       return item.pre_project.project_code;
//     case 'leave':
//       return item.first_name + ' ' + item.last_name;
//     case 'SousTraitant':
//       return item.raison_social;
//     case 'recrute':
//       return item.user.employee.first_name + ' ' + item.user.employee.last_name;
//     case 'created_by':
//       return item.created_by.employee.first_name + ' ' + item.created_by.employee.last_name;
//     case 'client':
//       return item.raison_social;
...
//       );
//     case 'percentage':
//       return (item.suivie / item.qte) * 100 + '%';
//     case 'bdc':
//       return item.bon_commande.num;
//     case 'soustraitant_facture':
//       return item.bon_commande.soustraitant.raison_social;
//     case 'workingHour':
//       return helpers.calculateDifference(item, item.employe.working_hours)[0];
//     case 'workingHourCustom':
//       return helpers.calculateDifference(item, item.employe.working_hours)[1];
//     case 'caisse':
//       return item.emetteur;
//     case 'tier':
//       return item.tier.commercial_name;
//     case 'carnet':
//       return item.carnet.numero;
//     default:
//       return '';
//   }
// };
// const ExportData = (filteredData, headers) => {
//   const data = filteredData.value.map((item) => {
//     const exportedItem = {};
//     headers.forEach((header) => {
//       if (header.isComplex) {
//         exportedItem[header.text] = complexTypeToString(item[header.value], header.type);
//       } else {
